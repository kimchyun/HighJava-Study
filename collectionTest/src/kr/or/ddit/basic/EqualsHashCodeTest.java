...
/*
- equals()메서드     => 두 객체의 내용이 같은 지를 비교하는 메서드  => 동등성
- hashCode()메서드   => 두 객체가 같은 객체인 지 비교하는 메서드    => 동일성

- HashTable, HashMap, HashSet과 같이 Hash로 시작하는 컬렉션 객체들은
  객체의 의미성의 동일성 비교를 위해서 hashCode()메서드를 호출하여 비교한다.
  그러므로 객체가 같은지 여부를 결정하려면 equals()메서드와 hashCode()메서드를 같이 재정의 해야 한다.
  
- hashCode()메서드에서 사용하는 '해싱 알고리즘'은 서로 다른 객체들에 대해 같은 hashCode값을 만들어 낼 수 있다.
...
// Person 객체의 id 값과 name 값이 모두 같으면 같은 객체로 처리되도록 하기
