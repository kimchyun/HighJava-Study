...
public class LprodMybatisTest {
	...
	public static void main(String[] args) {
		...
		// 2. mapper에 등록된 SQL문들 중 실행할 SQL문을 호출해서 원하는 작업을 수행한다.
		  ...
		  // 입력한 데이터를 VO객체에 저장한다. LprodVO lvo1 = new LprodVO();
		  lvo1.setLprod_id(lprodId); lvo1.setLprod_gu(lprodGu);
		  lvo1.setLprod_nm(lprodNm);
		  
		  SqlSession session = null; // SqlSession객체 변수 선언 try { //
		  SqlSessionFactory객체를 이용하여 SqlSession객체를 구한다.ㅣ // ==> openSession()메서드를 이용한다.
		  // 형식) SqlSessionFactory객체.openSession(논리값) // '논리값'이 true이면 AutoCommit이 활성화된
		  상태이고 // '논리값'이 생략되거나 false이면 AutoCommit이 비활성화된 상태이다. session =
		  sqlSessionFactory.openSession();
		  
		  // SqlSession객체를 이용하여 처리할 SQL 문을 호출하여 실행한다. // 형식)
		  SqlSession객체.insert("namespace속성값.id속성값", 파라미터클래스) // 반환값 : 작업에 성공한 레코드 수 //
		  int insertCnt = session.insert("lprod.insertLprod1", lvo1); int insertCnt =
		  session.insert("Lprod.insertLprod2", lvo1);
		  
		  if(insertCnt>0) { System.out.println("===== insert 작업 성공 =====");
		  
		  // SqlSession객체를 생성할 때 AutoCommit이 비활성화된 상태일 때에는 // commit을 직접 실행해야 한다.
		  session.commit(); } else { System.out.println("===== insert 작업 실패 ====="); }
		...
		// --------------------------------------------------------------
		  ...
		  /*
		  // 2-2. update연습 System.out.println("===== update 작업 시작 =====");
		  
		  System.out.print("수정할 Lprod_gu 입력 : "); String lprodGu2 = scan.next();
		  
		  System.out.print("새로운 Lprod_id 입력 : "); int lprodId2 = scan.nextInt();
		  
		  System.out.print("새로운 Lprod_nm 입력 : "); String lprodNm2 = scan.next();
		  
		  // 입력한 데이터를 VO에 저장하기 LprodVO lvo2 = new LprodVO();
		  lvo2.setLprod_gu(lprodGu2); lvo2.setLprod_id(lprodId2);
		  lvo2.setLprod_nm(lprodNm2);
		  
		  SqlSession session = null; try { // SqlSession객체 생성 session =
		  sqlSessionFactory.openSession();
		  
		  // SqlSession객체의 update()메서드를 이용해서 수정작업을 수행한다. // 형식)
		  SqlSession객체.update("namespace속성값.id속성값", 파라미터클래스) // 반환값 : 작업에 성공한 레코드 수 int
		  updateCnt = session.update("lprod.updateLprod", lvo2);
		  
		  if(updateCnt>0) { System.out.println("===== update 작업 성공 =====");
		  session.commit(); } else { System.out.println("===== update 작업 실패 ====="); }
		...
		// --------------------------------------------------------------
		 ...
		 System.out.println("===== delete 작업 시작 =====");
		  ...
		  try { 
		      ...
		      // SqlSession객체의 delete()메서드를 이용해서 삭제한다. 
			  // 형식) SqlSession객체.delete("namespace속성값.id속성값", 파라미터클래스) 
		      // 반환값 : 작업에 성공한 레코드 수 
		      intdeleteCnt = session.delete("lprod.deleteLprod", lprodGu3);
		  
		      if(deleteCnt>0) { System.out.println("===== delete 작업 성공 =====");
		      session.commit(); } else { System.out.println("===== delete 작업 실패 ====="); }
		...
		try {
			...
			// 여러개의 레코드 각각을 VO객체에 담은 후 이 VO데이터를 List에 추가해주는
			// 작업을 자동으로 수행한다.
        ...
        */
		...
		try {
			...
			if(lvo5==null) {
				System.out.println("검색한 데이터가 하나도 존재하지 않습니다.");
			} else {
