...
public class ResponseTest02 extends HttpServlet {
	...
	/*
	    - redirect방식
	       ...
	       * 응답 시 브라우저에게 '이동할 URL'을 전송하여 브라우저가 해당 URL로 이동하는 방식이다.
	       * 이동할 때의 전송방식은 GET방식으로 요청하여 이동한다.
	       * (URL의 길이에 제한이 있다.) 
	*/
		
	/*	
	    // redirect는 Request객체를 공유하지 못 한다. ( 브라우저에서 새롭게 요청하기 때문 )
		request.setAttribute("tel", "010-1234-5678");   // 보낼 데이터 셋팅
		
		// 이동하기 
		
		// redirect로 이동은 response객체의 sendRedirect()메서드를 사용한다.
	...
	*/
		...
		// URL경로에 한글이 포함될 경우에는 URLEncoder객체를 이용하여 인코딩해서 지정해 주어야 한다.
