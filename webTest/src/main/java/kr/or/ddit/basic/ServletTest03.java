...
2. 컨테이너 web.xml에 정의된 url패턴을 확인하여 어느 서블릿을 통해 처리해야 할 지를 검색한다.
     (이 때 서블릿이 로딩이 안 된 경우에는 로딩 작업을 한다. 처음 로딩 시에는 init()메서드를 호출한다.)
     (Servlet버전 3.0이상에서는 애노테이션(@WebServlet)으로 설정할 수 있다.)
3. Servlet Container는 개별 요청을 처리할 쓰레드를 생성하여 해당 서블릿 객체의 
   service()메서드를 호출한다.  (이 때 HttpServletRequest객체와 HttpServletResponse객체를 생성하여 파라미터로
   넘겨준다.)
...
public class ServletTest03 extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 방법1 ==> 부모 클래스인 HttpServlet의 service()메서드 위임하기
