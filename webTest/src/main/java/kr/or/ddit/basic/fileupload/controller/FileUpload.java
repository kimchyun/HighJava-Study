...
public class FileUpload extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		   - Servlet 3.0이상에서 새롭게 추가된 upload 관련 메소드
		...
		*/
		...
		IFileInfoService service = FileInfoServiceImpl.getInstance();  // Service객체 작성
	...
	1) 파일이 아닌 일반 데이터인 경우
	------------------alkerjkjsflkefjkf#@kjlsllsjf    ==> 각각의 Part를 구분하는 선
	...
	private String extractFileName(Part part) {
		...
		for(String item : items) {
			if(item.trim().startsWith("filename")) {  // 파일인지 여부 검사
				fileName = item.substring(item.indexOf("=") + 2, item.length()-1);  // 파일 이름 추출하기
			}
		}
